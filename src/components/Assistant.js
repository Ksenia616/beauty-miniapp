...
export default function Assistant() {
  ...
  if (step === "foundation") {
    return (
      <div className="flex flex-col items-center justify-center flex-1 bg-black text-white p-6 space-y-4">
        ...
        <StyledButton onClick={() => setStep("ff01")}>Светлая фарфорового оттенка, легко краснеет и едва загорает</StyledButton>
  ...
  if (step === "ff04") return renderProduct("TAN HONEY (FF04)", "Средне-темный беж с насыщенным теплым подтоном. Подходит загорелой или смуглой коже, идеально ложится на кожу после отпуска или с эффектом автозагара.", FF04, "https://ozon.ru/product/2389897299", "foundation");
